...
function startGame() {
    ...
    initializeDOM();
    if (!canvas) {
        console.error("Canvas not initialized. Aborting game start.");
        return;
    }
    loadUserData();
    resizeCanvas();
...
function initializeDOM() {
    ...
    earnButton = document.getElementById('earnButton') || createButton('earnButton', 'Earn');
    tasksButton = document.getElementById('tasksButton') || createButton('tasksButton', 'Tasks');
    boostButton = document.getElementById('boostButton') || createButton('boostButton', 'Boost');
    dailyRewardsButton = document.getElementById('dailyRewardsButton') || createButton('dailyRewardsButton', 'Daily Rewards');
    
    menuModal = document.getElementById('menuModal') || createModal('menuModal');
    dailyRewardDisplay = document.getElementById('dailyRewardDisplay') || createDisplay('dailyRewardDisplay');
    boostersModal = document.getElementById('boostersModal') || createModal('boostersModal');
    tasksModal = document.getElementById('tasksModal') || createModal('tasksModal');
    rewardTableModal = document.getElementById('rewardTableModal') || createModal('rewardTableModal');
    autoBotSuccessModal = document.getElementById('autoBotSuccessModal') || createModal('autoBotSuccessModal');
    autoBotEarningsModal = document.getElementById('autoBotEarningsModal') || createModal('autoBotEarningsModal');
    ...
    const nextRewardPageButton = document.getElementById('nextRewardPage');
    const prevRewardPageButton = document.getElementById('prevRewardPage');
    const closeRewardTableButton = document.getElementById('closeRewardTableButton');
    ...
    if (nextRewardPageButton) nextRewardPageButton.addEventListener('click', toggleRewardPage);
    if (prevRewardPageButton) prevRewardPageButton.addEventListener('click', toggleRewardPage);
    if (closeRewardTableButton) closeRewardTableButton.addEventListener('click', () => {
        rewardTableModal.style.display = 'none';
    });
    if (followUsButton) followUsButton.addEventListener('click', () => startTask('followX'));
    if (visitWebsiteButton) visitWebsiteButton.addEventListener('click', () => startTask('visitWebsite'));
    if (closeTasksModalButton) closeTasksModalButton.addEventListener('click', () => {
...
function loadDinoImages() {
    return new Promise((resolve, reject) => {
        function loadSingleImage(index) {
            return new Promise((resolve, reject) => {
                ...
                img.onerror = function() {
                    ...
                    reject(new Error(`Failed to load dino image ${index}`));
                };
            });
        }
        ...
        const imagePromises = [];
        for (let i = 1; i <= 5; i++) {
            imagePromises.push(loadSingleImage(i));
        }
...
function updateDinoImage() {
    ...
    console.log("Updating dino image:", currentDinoImage ? currentDinoImage.src : "No image");
    if (currentDinoImage) {
        currentDinoImage.onload = () => {
